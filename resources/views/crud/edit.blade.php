...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Edit Tiket #{{ $ticket->id }}</h1>
        <div class="flex space-x-3">
            <a href="{{ route('tickets.show', $ticket->id) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition duration-300">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('tickets.update', $ticket->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="movie_title" name="movie_title" value="{{ old('movie_title', $ticket->movie_title) }}" 
                    ...
                    <div>
                        ...
                        <select id="genre" name="genre" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('genre') border-red-500 @enderror" required>
                            ...
                            <option value="Action" {{ old('genre', $ticket->genre) == 'Action' ? 'selected' : '' }}>Action</option>
                            <option value="Comedy" {{ old('genre', $ticket->genre) == 'Comedy' ? 'selected' : '' }}>Comedy</option>
                            <option value="Drama" {{ old('genre', $ticket->genre) == 'Drama' ? 'selected' : '' }}>Drama</option>
                            <option value="Horror" {{ old('genre', $ticket->genre) == 'Horror' ? 'selected' : '' }}>Horror</option>
                            <option value="Romance" {{ old('genre', $ticket->genre) == 'Romance' ? 'selected' : '' }}>Romance</option>
                            <option value="Sci-Fi" {{ old('genre', $ticket->genre) == 'Sci-Fi' ? 'selected' : '' }}>Sci-Fi</option>
                            <option value="Thriller" {{ old('genre', $ticket->genre) == 'Thriller' ? 'selected' : '' }}>Thriller</option>
                            <option value="Animation" {{ old('genre', $ticket->genre) == 'Animation' ? 'selected' : '' }}>Animation</option>
                        </select>
                        @error('genre')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" id="duration" name="duration" value="{{ old('duration', $ticket->duration) }}" 
                    ...
                    <div>
                        ...
                        <select id="rating" name="rating" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('rating') border-red-500 @enderror">
                            ...
                            <option value="SU" {{ old('rating', $ticket->rating) == 'SU' ? 'selected' : '' }}>SU (Semua Umur)</option>
                            <option value="PG" {{ old('rating', $ticket->rating) == 'PG' ? 'selected' : '' }}>PG (Parental Guidance)</option>
                            <option value="PG-13" {{ old('rating', $ticket->rating) == 'PG-13' ? 'selected' : '' }}>PG-13</option>
                            <option value="R" {{ old('rating', $ticket->rating) == 'R' ? 'selected' : '' }}>R (17+)</option>
                        </select>
                        @error('rating')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="studio" name="studio" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('studio') border-red-500 @enderror" required>
                            ...
                            <option value="1" {{ old('studio', $ticket->studio) == '1' ? 'selected' : '' }}>Studio 1</option>
                            <option value="2" {{ old('studio', $ticket->studio) == '2' ? 'selected' : '' }}>Studio 2</option>
                            <option value="3" {{ old('studio', $ticket->studio) == '3' ? 'selected' : '' }}>Studio 3</option>
                            <option value="4" {{ old('studio', $ticket->studio) == '4' ? 'selected' : '' }}>Studio 4</option>
                            <option value="5" {{ old('studio', $ticket->studio) == '5' ? 'selected' : '' }}>Studio 5</option>
                        </select>
                        @error('studio')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" id="show_date" name="show_date" value="{{ old('show_date', $ticket->show_date) }}" 
                    ...
                    <div>
                        ...
                        <select id="show_time" name="show_time" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('show_time') border-red-500 @enderror" required>
                            ...
                            <option value="10:00" {{ old('show_time', $ticket->show_time) == '10:00' ? 'selected' : '' }}>10:00</option>
                            <option value="12:30" {{ old('show_time', $ticket->show_time) == '12:30' ? 'selected' : '' }}>12:30</option>
                            <option value="15:00" {{ old('show_time', $ticket->show_time) == '15:00' ? 'selected' : '' }}>15:00</option>
                            <option value="17:30" {{ old('show_time', $ticket->show_time) == '17:30' ? 'selected' : '' }}>17:30</option>
                            <option value="20:00" {{ old('show_time', $ticket->show_time) == '20:00' ? 'selected' : '' }}>20:00</option>
                            <option value="22:30" {{ old('show_time', $ticket->show_time) == '22:30' ? 'selected' : '' }}>22:30</option>
                        </select>
                        @error('show_time')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('status') border-red-500 @enderror" required>
                            <option value="available" {{ old('status', $ticket->status) == 'available' ? 'selected' : '' }}>Tersedia</option>
                            <option value="booked" {{ old('status', $ticket->status) == 'booked' ? 'selected' : '' }}>Dipesan</option>
                            <option value="paid" {{ old('status', $ticket->status) == 'paid' ? 'selected' : '' }}>Dibayar</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
            ...
            <div class="mt-6 pt-6 border-t">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" id="customer_name" name="customer_name" value="{{ old('customer_name', $ticket->customer_name) }}" 
                    ...
                    <div>
                        ...
                        <input type="email" id="customer_email" name="customer_email" value="{{ old('customer_email', $ticket->customer_email) }}" 
                    ...
                    <div>
                        ...
                        <input type="tel" id="customer_phone" name="customer_phone" value="{{ old('customer_phone', $ticket->customer_phone) }}" 
            ...
            <div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
                <div class="flex items-center">
                    ...
                    <p class="text-sm text-blue-800">
                        <strong>Catatan:</strong> Tiket ini sudah dibayar. Pastikan perubahan yang dilakukan sudah sesuai.
                    </p>
                </div>
            </div>
            @endif
            ...
            <div class="mt-8 flex justify-end space-x-4">
                <a href="{{ route('tickets.show', $ticket->id) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg transition duration-300">
                    Batal
                </a>
...
document.getElementById('status').addEventListener('change', function(e) {
    ...
    if (originalStatus === 'paid' && newStatus !== 'paid') {
        if (!confirm('Anda akan mengubah status tiket yang sudah dibayar. Apakah Anda yakin?')) {
